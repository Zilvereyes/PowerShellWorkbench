...
- Copy and adapt [assets/templates/provider-switch-transaction.ps1.tmpl](assets/templates/provider-switch-transaction.ps1.tmpl) for reversible provider configuration changes. Its read-only resume decision verifies phase, freshness, paths, hashes, provider/model/context identity, owned keys, and owned state before allowing apply, post-check, or restore continuation.
