...
- Use `../../scripts/Get-PowerShellWorkbenchCatalogMigrationPreview.ps1` to assess a schema 1.0 catalog before migration.
  Supply the expected manifest hash, an explicit destination, and a scoped allowed write root. The previewer never writes
  or launches Codex: verified schema 1.0 evidence returns a hash-bound regeneration plan, schema 1.1 returns `NO_CHANGE`,
  and unknown or drifted evidence returns exact blocking gates. Applying that plan is a separate explicitly authorized operation.
