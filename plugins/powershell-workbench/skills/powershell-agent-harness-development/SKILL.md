...
- Copy and adapt [assets/templates/provider-switch-transaction.ps1.tmpl](assets/templates/provider-switch-transaction.ps1.tmpl) for reversible provider configuration changes. Its read-only resume decision verifies phase, freshness, paths, hashes, provider/model/context identity, loopback endpoint, wire API, local-model catalog, owned keys, canonical owned state, managed-state absence, and restore evidence before allowing apply, managed commit, post-check, or restore continuation. `ProviderProfileWritten` and `DesktopLifecycleInterrupted` return `ManagedCommitRequired` without repeating the provider edit or invoking Desktop lifecycle work.
