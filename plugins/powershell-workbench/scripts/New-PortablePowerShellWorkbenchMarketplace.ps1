...
$plan=[pscustomobject][ordered]@{SchemaVersion='1.0';State='PREVIEW';Mode=$mode;MarketplaceRoot=$destinationRoot;PluginPath=$pluginDestination;MarketplacePath=$marketplacePath;ExistingPlugin=$pluginExists;ExistingMarketplace=$marketplaceExists;WritePerformed=$false;RestorePerformed=$false;TransportPerformed=$false}
if(-not$PSCmdlet.ShouldProcess($destinationRoot,"Perform reversible portable marketplace $mode")){return $plan}

$marketplace=[ordered]@{name='powershell-workbench';interface=[ordered]@{displayName='PowerShell Workbench'};plugins=@([ordered]@{name='powershell-workbench';source=[ordered]@{source='local';path='./plugins/powershell-workbench'};policy=[ordered]@{installation='AVAILABLE';authentication='ON_INSTALL'};category='Productivity'})}
...
try{
    ...
    if([string]$stagedDocument.name-cne'powershell-workbench'-or@($stagedDocument.plugins).Count-ne 1){throw 'Staged marketplace validation failed.'}
