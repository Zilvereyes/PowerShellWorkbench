...
function Stop-Approval { param([string[]]$Gates,[string]$State='CONFLICT') $result=[pscustomobject][ordered]@{SchemaVersion='1.0';State=$State;Eligible=$false;FailedGates=$Gates;WritePerformed=$false;ToolExecutionPerformed=$false;TransportPerformed=$false};if(-not $NoThrow){throw "Read-only tool approval is $State`: $($Gates -join ', ')."};$result }

if($ExpiresAt.ToUniversalTime() -le $CreatedAt.ToUniversalTime()){return (Stop-Approval -Gates @('ApprovalWindow') -State 'UNKNOWN')}
...
if($proposalGates.Count -gt 0){return (Stop-Approval -Gates $proposalGates -State $proposalDecision.State)}
$approvedRoot=Get-NormalizedPath -Path $proposalDecision.AllowedRoot
if(-not $approvedRoot -or -not(Test-PathChain -Path $proposalDecision.TargetPath -Root $approvedRoot)){return (Stop-Approval -Gates @('TargetReparseSafe'))}
$targetSnapshot=Read-Snapshot -LiteralPath $proposalDecision.TargetPath -MaximumBytes $MaximumTargetBytes
if(-not $targetSnapshot.Path.StartsWith($approvedRoot+[IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase) -or -not(Test-PathChain -Path $targetSnapshot.Path -Root $approvedRoot)){return (Stop-Approval -Gates @('TargetBoundaryDrift'))}
...
if(-not $evidenceRoot -or $evidenceRoot -ieq [IO.Path]::GetPathRoot($evidenceRoot) -or -not(Test-Path -LiteralPath $evidenceRoot -PathType Container)){return (Stop-Approval -Gates @('AllowedEvidenceRoot'))}
if(-not $normalizedOutput -or -not $normalizedOutput.StartsWith($evidenceRoot+[IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase)){return (Stop-Approval -Gates @('OutputPathWithinEvidenceRoot'))}
if(-not(Test-PathChain -Path $normalizedOutput -Root $evidenceRoot)){return (Stop-Approval -Gates @('OutputPathReparseSafe'))}
$parent=Split-Path -Parent $normalizedOutput;if(-not(Test-Path -LiteralPath $parent -PathType Container)){return (Stop-Approval -Gates @('OutputParentExists') -State 'UNKNOWN')}
if($Approve){if(-not(Test-PathChain -Path $normalizedOutput -Root $evidenceRoot)){return (Stop-Approval -Gates @('OutputPathReparseSafe'))};$stream=[IO.File]::Open($normalizedOutput,[IO.FileMode]::CreateNew,[IO.FileAccess]::Write,[IO.FileShare]::Read);try{$stream.Write($approvalBytes,0,$approvalBytes.Length)}finally{$stream.Dispose()}}
