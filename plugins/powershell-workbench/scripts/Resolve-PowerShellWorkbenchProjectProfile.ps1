...
try{$profile=Get-Content -LiteralPath $profilePathResolved -Raw|ConvertFrom-Json}catch{throw "Project profile is invalid JSON: $($_.Exception.Message)"}
if([string]$profile.schemaVersion -ne '1.0'){throw 'Project profile schemaVersion must be 1.0.'}
...
$projectRoot=Resolve-ProfileRoot -Value ([string]$profile.project.root) -Label 'Project'
$projectPrefix=$projectRoot.TrimEnd([char[]]'\\/')+[IO.Path]::DirectorySeparatorChar
$components=foreach($component in @($profile.components)){
...
$paths=[ordered]@{}
foreach($property in @($profile.paths.PSObject.Properties)){if([IO.Path]::IsPathRooted([string]$property.Value) -and -not $AllowAbsoluteRoots){throw "Configured path '$($property.Name)' must be relative."};$paths[$property.Name]=[IO.Path]::GetFullPath((Join-Path $projectRoot ([string]$property.Value)))}
$result=[pscustomobject]@{SchemaVersion='1.0';ProfilePath=$profilePathResolved;ProjectName=[string]$profile.project.name;ProjectRoot=$projectRoot;ProjectRootExists=(Test-Path -LiteralPath $projectRoot -PathType Container);Components=@($components);WindowsTargets=@($profile.targets.windows);Paths=[pscustomobject]$paths}
