...
function Write-OperatorBlock {
    ...
    Write-Host ("`n========== {0} ==========" -f $State) -ForegroundColor $color
    Write-Host $Message -ForegroundColor $color
    Write-Host ('=' * (22 + $State.Length)) -ForegroundColor $color
}
...
if($AnalyzeOnly){$timeline.Add('ANALYZE_ONLY process was not started.');Write-OperatorBlock -State 'FAERDIG' -Message "[$StepId] analyse udfoert; processen blev ikke startet."}else{
    try{
        ...
        try{
            & $resolvedFile @ArgumentList 2>&1|ForEach-Object {$line=Protect-OperatorText ([string]$_);$timeline.Add("OUTPUT $line");Write-Host $line}
    ...
    }catch{$executionError=$_.Exception.Message;Write-OperatorBlock -State 'FEJL' -Message "[$StepId] kunne ikke køres: $executionError";$timeline.Add("FEJL $executionError")}
}
