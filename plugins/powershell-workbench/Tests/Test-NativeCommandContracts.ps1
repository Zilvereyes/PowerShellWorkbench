...
try{
    ...
    if(-not $analyze.AnalyzeOnly -or $analyze.TargetMutation -or $analyze.MutationIntent -ne 'None' -or -not(Test-Path -LiteralPath $analyze.ReportPath)){throw 'Analyze-only contract failed.'}
    ...
    if($run.MutationIntent -ne 'Expected'){throw 'Native command mutation intent was not preserved.'}
