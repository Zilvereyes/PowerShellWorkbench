...
try{
    ...
    foreach($profile in @('RecoveryToolkit','WingetDownloader')){
        $item=& $generator -Kind PesterContract -Name 'Fixture' -Destination $tempRoot -Profile $profile -ProjectRoot $tempRoot -Confirm:$false
        if($item.Name -ne 'Test-Fixture.Contract.ps1'){throw "Profile $profile generated a non-discoverable test name: $($item.Name)"}
