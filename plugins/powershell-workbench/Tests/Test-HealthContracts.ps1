...
try{
    $source=New-FixtureRoot source;$cache=Copy-Fixture $source cache;$distribution=[pscustomobject]@{Name='personal';SourcePath=$source;CachePath=$cache};$before=Get-Snapshot @($source,$cache)
    ...
    Assert-GatesExactly $empty @('DistributionEvidenceRequired') 'Missing distribution evidence gate changed.'

    $alias=&$validator -Distribution ([pscustomobject]@{Name='alias';SourcePath=$source;CachePath=$source}) -ExpectedVersion '0.7.4' -NoThrow
    Assert-GatesExactly $alias @('DistributionSourceCacheDistinct') 'Source/cache alias gate changed.'

    $duplicates=&$validator -Distribution @($distribution,$distribution) -ExpectedVersion '0.7.4' -NoThrow
    Assert-GatesExactly $duplicates @('DistributionNameUnique','DistributionSourcePathUnique','DistributionCachePathUnique') 'Duplicate distribution gates changed.'
    ...
    Assert-GatesExactly $drift @('DistributionTreeIdentity') 'Tree drift gate changed.'
    Copy-Item -LiteralPath (Join-Path $source 'scripts\fixture.ps1') -Destination (Join-Path $cache 'scripts\fixture.ps1') -Force

    $unknownSource=New-FixtureRoot unknown-source -Version 'unknown';$unknownCache=Copy-Fixture $unknownSource unknown-cache
    ...
    Assert-GatesExactly $unknown @('DistributionSourceVersionValid','DistributionCacheVersionValid') 'Unknown-version gates changed.'
    ...
    Assert-GatesExactly $missingVersion @('DistributionSourceVersionValid','DistributionTreeIdentity') 'Missing-version gates changed.'
    ...
    Assert-GatesExactly $missing @('DistributionSourceExists') 'Missing-source gate changed.'
    ...
    Assert-GatesExactly $stale @('CatalogFreshness') 'Stale catalog gate changed.'
    ...
    Assert-GatesExactly $unknownSchema @('CatalogSchemaVersion') 'Unknown catalog schema gate changed.'
    ...
    Assert-GatesExactly $catalogDrift @('CatalogArtifactHash','CatalogModelEntry') 'Catalog drift gates changed.'
    $required=&$validator -Distribution $distribution -ExpectedVersion '0.7.4' -RequireCatalog -NoThrow
    Assert-GatesExactly $required @('CatalogEvidenceRequired') 'Missing catalog evidence gate changed.'
